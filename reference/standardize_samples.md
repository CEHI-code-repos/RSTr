...
#> Error in age_standardize(minsample, std_pop, margin = age_margin): unused argument (margin = age_margin)
# age-standardize only by the first two age groups
samples_3554 <- age_standardize(minsample, std_pop[1:2], age_margin, groups = 1:2)
#> Error in `*tmp*`$params: $ operator is invalid for atomic vectors
...
#> Error in age_standardize(minsample, std_pop, new_name = "35-64", margin = age_margin,     bind_new = TRUE): unused arguments (margin = age_margin, bind_new = TRUE)
```
