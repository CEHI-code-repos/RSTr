...
So now, what if we want to get estimates averaged across all years? In this case, we will expand our `theta` and `pop` arrays using the `group_aggregate()` and `pop_aggregate()` functions, respectively:
