# 03: Initializing Your Model

## Overview

The `initialize_*()` functions
...
## Arguments of the `initialize_*()` function

All `initialize_*()` functions provide several arguments:
...
- `show_plots`: If set to `FALSE`, hides diagnostic plots from initial
  setup;

- `method`: Chooses whether the event data is either Binomial
  (`"binomial"`) or Poisson (`"poisson"`) distributed. By default,
  `RSTr` uses Binomial updates for the event data;
...
- `inits`: This is a `list` of initial values for each parameter. This
  can be specified by the user or generated by default; and
...
most arguments.
[`initialize_ucar_restricted()`](../reference/initialize_ucar.md) and
[`initialize_mstcar()`](../reference/initialize_ucar.md) have additional
arguments only used by them:

- `A`: In the restricted UCAR model, describes the limit of the
...
`inits` is a `list` specifying the starting values for each parameter of
the model. Each parameter can have an initial value specified. Here are
the possible initial value parameters for the MSTCAR model:
...
information related to parameter priors. The following are all priors
used in the MSTCAR model:
...
to `inits`, you don’t have to specify priors for *all* parameters if you
only want to specify some of them - any undefined priors will be defined
by the default values. Any values included in `priors` that aren’t
aligned with the above names will be ignored.
...
estimates, set by default to `1234`.
...
this vignette, we explored each argument of the `initialize_*()`
functions and how to appropriately choose values for each argument.
